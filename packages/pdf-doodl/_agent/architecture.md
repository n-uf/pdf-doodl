...
Doodl (`@n-uf/pdf-doodl`) is a **vanilla-first** canvas drawing and annotation library. It provides shape drawing capabilities with full JSON serialization, designed to work with any framework or no framework at all.
...
```tsx
import { createDoodl, type Doodl } from "@n-uf/pdf-doodl";
